...
let arrayObj = miArray.map(function(x) {
...
function limpiarTabla() {
    let cliente = arrayObj.filter(x => '_edad' in x);
    let empleado = arrayObj.filter(x => '_cuil' in x);
...
function generarTabla() {
    ...
    if(!cuerpoTabla.childElementCount>=0){
        arrayObj.forEach(item => {
            if(selectTipo.value == "clientes" && item instanceof Cliente){
                cargarFilas(item);
            }
            else{
                if(selectTipo.value == "empleados" && item instanceof Empleado){
                    cargarFilas(item);
                }
                else{
...
function cargarFilas(item){
    let cuerpoTabla = document.querySelector("#cuerpoTabla");
    let datos = [item.Id,item.Apellido,item.Nombre,item.Edad,item.Sexo,item.Cuil,item.Sueldo];
...
function filtradoTabla(obj) {
    ...
    if(!cuerpoTabla.childElementCount>0){
        if(obj == "clientes"){
            let clientes = arrayObj.filter(x => '_edad' in x);
            ...
            let empleados = arrayObj.filter(x => '_cuil' in x);
...
function calcular() {
    ...
    switch (x) {
        case "clientes":
            clientes = arrayObj.filter(x => '_edad' in x);
            clientes.forEach(item => {
                acumulador += parseInt(item.Id);
            });
        ...
        case "empleados":
            empleados = arrayObj.filter(x => '_cuil' in x);
            empleados.forEach(item => {
                acumulador += parseFloat(item.Id);
            });
        ...
        default:
            ...
            promedio = parseFloat(acumulador)/arrayObj.length;
            document.getElementById("promId").value = promedio;
            break;
    }
}
...
alta.addEventListener('click', agregarObj);

function insertarNuevo() {
    let indice = arrayObj.length-1;
    let newObj = arrayObj[indice];
    cargarFilas(newObj);
}
...
altas.addEventListener('click', agregarObj);

function agregarObj(){
    let id = arrayObj.reduce( (acc,item) => {
        let i = Math.max(acc, item.Id);
        return i += 1;
    },0);
        ...
        if(id==arrayObj[i].Id){
            if(id>0 && nombre!="" && apellido!=""){
                if(document.getElementById("fSelect").value == "cliente"){
                    ...
                    let sexo = document.getElementById("fInput2").value;
                    if(edad>0 && sexo!=""){
                        arrayObj.splice(i,1,new Cliente(id,nombre,apellido,edad,sexo));
                        break;
                    }
                }
                else{
                    ...
                    let sueldo = parseInt(document.getElementById("fInput2").value);
                    if(cuil>0 && sueldo>0){
                        arrayObj.splice(i,1,new Empleado(id,nombre,apellido,cuil,sueldo));
                        break;
                    }
                }
            }
